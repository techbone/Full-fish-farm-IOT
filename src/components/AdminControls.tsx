...
import { PondCommand } from '../types/pond';

interface AdminControlsProps {
  pondId: number;
}

const AdminControls: React.FC<AdminControlsProps> = ({ pondId }) => {
  const { sendCommand } = usePond();
  const [isLoading, setIsLoading] = useState(false);
  const [lastCommand, setLastCommand] = useState<string | null>(null);

  const handleCommand = async (command: PondCommand, description: string) => {
    setIsLoading(true);
    try {
