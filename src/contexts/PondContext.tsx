...
import { Pond, PondState, PondCommand } from '../types/pond';
import { WebSocketMessage } from '../types/sensor';
import * as api from '../services/api';
import { useAuth } from './AuthContext';

interface PondContextType extends PondState {
  fetchPonds: () => Promise<void>;
  selectPond: (pond: Pond) => void;
  updatePondData: (message: WebSocketMessage) => void;
  sendCommand: (pondId: number, command: PondCommand) => Promise<void>;
}
...
type PondAction =
  | { type: 'FETCH_START' }
  | { type: 'FETCH_SUCCESS'; payload: Pond[] }
  | { type: 'FETCH_ERROR'; payload: string }
  | { type: 'SELECT_POND'; payload: Pond }
  | { type: 'UPDATE_POND'; payload: WebSocketMessage }
  | { type: 'COMMAND_SUCCESS' }
  | { type: 'COMMAND_ERROR'; payload: string };

const pondReducer = (state: PondState, action: PondAction): PondState => {
  switch (action.type) {
    case 'FETCH_START':
...
export const PondProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updatePondData = (message: WebSocketMessage) => {
    dispatch({ type: 'UPDATE_POND', payload: message });
  };

  const sendCommand = async (pondId: number, command: PondCommand) => {
